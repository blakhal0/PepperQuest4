...
class chatbrasswolfinnkeeper(default_cmds.MuxCommand):
	key = "talk innkeeper"
	...
	#Stay
			...
			if not "The King of Thieves" in self.caller.db.accolades:
				price = int(self.caller.db.lvl) * 3000
			else:
			...
			if self.caller.db.gold < int(price):
				...
				if not "The King of Thieves" in self.caller.db.accolades:
					self.caller.msg("|mInnkeeper|n says: Let's see here, that'll be %d for the night. A bit pricey I know, but as you can see, we're quite booked." % (int(price)))
				else:
...
class BrassWolfInnkeeperCmdSet(CmdSet):
	key = "BrassWolfInnkeeperCmdSet"
	def at_cmdset_creation(self):
		self.add(chatbrasswolfinnkeeper())

class brasswolfinnkeeper(DefaultObject):
	def at_object_creation(self):
		...
		self.cmdset.add_default(BrassWolfInnkeeperCmdSet, permanent=True)
