...
class chatgreeter(default_cmds.MuxCommand):
	...
	def func(self):
		...
		self.caller.msg("|mGreeter|n says: One more thing, there's a whole lotta merchants in there, no need to call each and every single individual one of them by their proper name. You can just |cTalk Merchant|n if you want to take a look at their wares. Don't know why that is, it just is, so don't go raising a fuss about it. Lazy ass programmer probably.")
		while 1 > 0:
