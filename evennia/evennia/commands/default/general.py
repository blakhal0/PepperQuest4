    ...
    def func(self):
        ...
        if not items:
            ...
            for item in items:
                table.add_row(
                    f"|C{item.name}|n",
                    "{}|n".format(utils.crop(raw_ansi(item.db.desc), width=120) or ""),
                )
...
class CmdGive(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        if not to_give.location == caller:
            caller.msg("You are not holding %s." % to_give.key)
            return
        if not to_give.tags.get("givable"):
            caller.msg("You cannot give this item.")
            return
        if not target.has_account:
            caller.msg("Hey, what are you trying to pull here? You can only give items to other players.|/You can blame Honswlos for this.")
            return
        #fork for items that are not single instance
        if not to_give.tags.get("singlegive"):
            quantity = yield("|/How many do you want to give? You have %d %s." % (to_give.db.qty, to_give.key))
            if not quantity.isnumeric():
                caller.msg("|rThat doesn't look like a number. No funny business!|n")
                return
            if int(quantity) > int(to_give.db.qty):
                caller.msg("|/|rYou don't have enough %s to give %d.|n" % (to_give.key, int(quantity)))
                return
            #calling at_before_give hook method
            if not to_give.at_before_give(caller, target, quiet=True):
                return
            #check if receiving player has some of the thing.
            targetitem = target.search(to_give.key, location=target, quiet=True)
            if targetitem:
                #receiving player has item, increase receiving player quantity
                targetitem[0].db.qty += int(quantity)
                to_give.db.qty -= int(quantity)
                caller.msg("You give %d %s to %s." % (int(quantity), to_give.key, target.key))
                target.msg("%s gives you %d %s." % (caller.key, int(quantity), to_give.key))
                #check if player gave all of their item to receiving player
                if to_give.db.qty == 0:
                    caller.msg("|/You gave away all of your %s." % (to_give.key))
                    to_give.delete()
                    return
                return
            if not targetitem:
                #receiving player does not have item, spawn item to receiving player inventory
                name = to_give.key.lower().replace(" ", "")
                pg_proto = {
                "key": to_give.key,
                "typeclass": "typeclasses.items.%s" % (name),
                "qty": int(quantity),
                "location": target
                }
                spawn(pg_proto)
                ...
                if to_give.db.qty == 0:
                    ...
                    return
                return
        #fork for items that are single instance.
        else:
            if not to_give.at_before_give(caller, target, quiet=True):
                return
            success = to_give.move_to(target, quiet=True)
