...
#Item is in a list type attribute attached to the accessing object
def inlist(accessing_obj, accessed_obj, *args, **kwargs):
    attribute = args[0].strip()
    listitem = args[1].strip()
    if listitem in accessing_obj.attributes.get(attribute):
        return True
    else:
        return False

#A monster has been defeated
def mondef(accessing_obj, accessed_obj, monstername):
    if monstername in accessing_obj.attributes.get("monsterstats"):
        return True
    else:
        return False

#A monster has NOT been defeated
def monnotdef(accessing_obj, accessed_obj, monstername):
    if not monstername in accessing_obj.attributes.get("monsterstats"):
        return True
    else:
        return False

#More than X number of a monster has been defeated
def mondefgtr(accessing_obj, accessed_obj, monstername, defeatedqty):
    if not monstername in accessing_obj.attributes.get("monsterstats"):
        return False
    elif accessing_obj.db.monsterstats[monstername]["killed"] > int(defeatedqty):
        return True
    else:
        return False

#More than 1 but less than X number of a monster has been defeated
def mondeflss(accessing_obj, accessed_obj, monstername, defeatedqty):
    if not monstername in accessing_obj.attributes.get("monsterstats"):
        return False
    elif accessing_obj.db.monsterstats[monstername]["killed"] < int(defeatedqty):
        return True
...
#Monster was not defeated or defeated is less than
def monnotdeforlss(accessing_obj, accessed_obj, monstername, defeatedqty):
    if not monstername in accessing_obj.attributes.get("monsterstats"):
        return True
    elif accessing_obj.db.monsterstats[monstername]["killed"] < int(defeatedqty):
        return True
